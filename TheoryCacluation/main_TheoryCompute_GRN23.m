%%%%% Calculate the final functional sizes of the GRN23 (Tissue Specific), PPI and metabolic
...
%%%%% denote the final functional sizes of the GRN23, PPI and metabolic
%%%%% netwokrs.
...
Coupling_P = (1-0.1911);%0.8342574; %Gene23
Coupling_G = 1-0.7433;%1-0.71;
PPI_FuncSize = zeros(length(0.01:0.01:1),1);

% Coupling_P = 11741/15906;
% Coupling_G = 11741/13521;
...
fileID = fopen('Simulations/Gene23_Simulation_WholeRobust_fpm1.txt');
Gene_FuncSizeSim = textscan(fileID,'%f');
fclose(fileID);
fileID = fopen('Simulations/PPI23_Simulation_WholeRobust_fpm1.txt');
PPI_FuncSizeSim = textscan(fileID,'%f');
fclose(fileID);
fileID = fopen('Simulations/Meta23_Simulation_WholeRobust_fpm1.txt');
...
tit1 = title('d','Fontsize',12, 'FontWeight','bold');
set(tit1,'Position',[-0.2 1.05 1.00011]);
ylim([0.0000099 1]);
gcalegend = legend('Lung GRN','PPI','Metabolic',[380 100 0.15 0.03]);
legend('boxoff');
...
FuncFrac23 = FuncFrac; 
PPI_FuncSize23 = PPI_FuncSize; 
Meta_Func23 = Meta_Func;
save Theo_Simu_Gene23 FuncFrac23 PPI_FuncSize23 Meta_Func23
