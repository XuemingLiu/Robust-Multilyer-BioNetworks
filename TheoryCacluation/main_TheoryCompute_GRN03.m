%%%%% Calculate the final functional sizes of the GRN03, PPI and metabolic
...
%%%%% denote the final functional sizes of the GRN03, PPI and metabolic
%%%%% netwokrs.

clear
%%%% Read the joint degree distribution of the GRN
load('Deg_NetA_03.mat')
fileID = fopen('Gene03_DegDis.txt');
...
Coupling_P = (1-0.1546);%(1-0.1360041);
Coupling_G = (1-0.7614);
...
fileID = fopen('Simulations/Gene03_Simulation_WholeRobust_fpm1.txt');
Gene_FuncSizeSim = textscan(fileID,'%f');
fclose(fileID);
fileID = fopen('Simulations/PPI03_Simulation_WholeRobust_fpm1.txt');
PPI_FuncSizeSim = textscan(fileID,'%f');
fclose(fileID);
fileID = fopen('Simulations/Meta03_Simulation_WholeRobust_fpm1.txt');
...
tit1 = title('b','Fontsize',12, 'FontWeight','bold');
set(tit1,'Position',[-0.2 1.05 1.00011]);
ylim([0.0000099 1]);
gcalegend = legend('Forebrain GRN','PPI','Metabolic',[380 300 0.15 0.03]);
legend('boxoff');
