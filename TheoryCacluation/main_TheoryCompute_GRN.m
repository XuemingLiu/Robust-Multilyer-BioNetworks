%%%%% Calculate the final functional sizes of the GRN, PPI and metabolic
...
%%%%% denote the final functional sizes of the GRN, PPI and metabolic
%%%%% netwokrs.

clear
%%%% Read the joint degree distribution of the GRN
load('Deg_NetA.mat')
fileID = fopen('Gene_DegDis.txt');
...
Coupling_P = 1-0.3710;
Coupling_G = 1-0.6077;
% Coupling_P = 10255/15906;
% Coupling_G = 10255/18566;
...
PPI_Func_temp1(:,1) = zeros(50,1);
    ...
    fP2M = 1;
        Meta_Func(num,fP2M) = FuncSizeMeta_FP2M(PPI_FuncSize(num), P_Meta2PPI, qB, Meta_DegDis, fP2M);
   
    
%save Meta_Func_DifferentThresholds Meta_Func 
end
...
figure(3)
gca = subplot(2,2,1);
gca1s = plot((0.05:0.05:1), Gene_FuncSizeSim{1,1}(5:5:100),'s', 'MarkerSize',5,  'LineWidth',1);
hold on 
%set(gca1s, 'Color', [170 0 255]./255)
set(gca1s, 'Color', [139,0,139]./255)

gca2s = plot((0.05:0.05:1), PPI_FuncSizeSim{1,1}(5:5:100),'o', 'MarkerSize',5,'LineWidth',1);
set(gca2s, 'Color', [0 50 0]./255)
gca3s = plot((0.05:0.05:1), Meta_FuncSizeSim{1,1}(5:5:100),'*', 'MarkerSize',5,'LineWidth',1);
...
ylim([0.0000099 1]);
gcalegend = legend('General GRN','PPI','Metabolic',[130 330 0.15 0.03]);
legend('boxoff');
...
hold off
%print(gcf, '-depsc2', 'RealNets_Theory_Simulation.eps')