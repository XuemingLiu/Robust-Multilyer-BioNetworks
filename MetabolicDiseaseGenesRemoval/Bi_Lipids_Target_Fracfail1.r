...
EssentialGene_ori <- read.table("lipids_in_GRN.txt",header = FALSE)$V1
...
#############################################     Metabolic network
    ...
    write.table(Size_Func_Aver, "Gene_Target_Lipids_Aver.txt",row.names = FALSE, col.names = FALSE)
  Size_FuncPPI_Aver <- rowSums(Size_FuncPPI)/N_PPI/k 
    write.table( Size_FuncPPI_Aver, "PPI_Target_Lipids_Aver.txt",row.names = FALSE, col.names = FALSE)
 
    
    Size_FuncB_ALL = Size_FuncB/N_Meta
    write.table(Size_FuncB_ALL, "Meta_Target_Lipids_Fraction01To1.txt",row.names = FALSE, col.names = FALSE)
     Size_FuncMeta_Aver <-  array(rep(0,pmax*10), dim=c(pmax,10))
    for(i_fp2m in 1:10)
    {
    	Size_FuncMeta_Aver[,i_fp2m] <- rowSums(Size_FuncB[,,i_fp2m])/N_Meta/k
    }
    write.table(Size_FuncMeta_Aver, "Meta_Target_Lipids_fpm01To1_Aver.txt",row.names = FALSE, col.names = FALSE)    
