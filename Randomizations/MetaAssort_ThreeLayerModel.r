...
{	
	...
	for(i_rep_rewire in 1:N_Meta_link)
	{
		...
		if(Endpoints22[1,1]!=Endpoints22[2,1] && Endpoints22[1,1]!=Endpoints22[2,2] && Endpoints22[1,2]!=Endpoints22[2,1] && Endpoints22[1,2]!=Endpoints22[2,2]) 
		{
			...
			End1ConnectEnd2 <- which(EndPints4[2] %in% as_ids(neighbors(Meta_Net,EndPints4[1], mode=c("all"))))
			End3ConnectEnd4 <- which(EndPints4[4] %in% as_ids(neighbors(Meta_Net,EndPints4[3], mode=c("all"))))
	    	if(length(End1ConnectEnd2)==0 && length(End3ConnectEnd4)==0)
	   		 {
	    		Meta_Net <- delete.edges(Meta_Net, Edge2_ids )
				Meta_Net <- add_edges(Meta_Net, c(EndPints4[1], EndPints4[2]))
				Meta_Net <- add_edges(Meta_Net, c(EndPints4[3], EndPints4[4]))
			}
		}
	}
...
#############################################     Metabolic network
    ...
    write.table(Size_FuncGene_Aver, "Gene_MetaAssort.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncPPI_Aver <- rowSums(Size_FuncPPI)/N_PPI/k_num
    write.table(Size_FuncPPI_Aver, "PPI_MetaAssort.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncMeta_Aver <-  array(rep(0,pmax*10), dim=c(pmax,10))
    for(i_fp2m in 1:10)
    {
    	Size_FuncMeta_Aver[,i_fp2m] <- rowSums(Size_FuncMeta[,,i_fp2m])/N_Meta/k_num
    }
    write.table(Size_FuncMeta_Aver, "Meta_MetaAssort_fpm01To1.txt",row.names = FALSE, col.names = FALSE)
