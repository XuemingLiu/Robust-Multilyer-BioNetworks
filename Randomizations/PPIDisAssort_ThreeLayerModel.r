...
for(k_num in 1:k_max)
{
	...
	for(i_rep_rewire in 1:N_PPI_link)
	{
		...
		{
			EndPints4 <- EndpointsArrary[order(-PPI_Degree[EndpointsArrary])]		
			End1ConnectEnd4 <- which(EndPints4[4] %in% as_ids(neighbors(g_PPI,EndPints4[1], mode=c("all"))))
			End2ConnectEnd3 <- which(EndPints4[2] %in% as_ids(neighbors(g_PPI,EndPints4[3], mode=c("all"))))
	    	if(length(End1ConnectEnd4)==0 && length(End2ConnectEnd3)==0)
	    	{
	    		g_PPI <- delete.edges(g_PPI, Edge2_ids )
				g_PPI <- add_edges(g_PPI, c(EndPints4[1], EndPints4[4]))
				g_PPI <- add_edges(g_PPI, c(EndPints4[2], EndPints4[3]))
			}
		}
	}
...
#############################################     Metabolic network
    ...
    write.table(Size_FuncGene_Aver, "Gene_PPIDisAssort.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncPPI_Aver <- rowSums(Size_FuncPPI)/N_PPI/k_num
    write.table(Size_FuncPPI_Aver, "PPI_PPIDisAssort.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncMeta_Aver <-  array(rep(0,pmax*10), dim=c(pmax,10))
    for(i_fp2m in 1:10)
    {
    	Size_FuncMeta_Aver[,i_fp2m] <- rowSums(Size_FuncMeta[,,i_fp2m])/N_Meta/k_num
    }
    write.table(Size_FuncMeta_Aver, "Meta_PPIDisAssort_fpm01To1.txt",row.names = FALSE, col.names = FALSE)
