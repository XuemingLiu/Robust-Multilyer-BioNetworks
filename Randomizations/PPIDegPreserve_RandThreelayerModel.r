...
g_Gene_read <- g_ori
g_PPI_Ori<- read.graph("PPI_DegreePreserved_Randomized.txt", format = c("edgelist"),directed=FALSE)
...
#############################################     Metabolic network
    ...
    write.table(Size_FuncGene_Aver, "Gene_PPIDegPreserve_ThreeLayerModels.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncPPI_Aver <- rowSums(Size_FuncPPI)/N_PPI/k_num
    write.table(Size_FuncPPI_Aver, "PPI_PPIDegPreserve_ThreeLayerModels.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncMeta_Aver <-  array(rep(0,pmax*10), dim=c(pmax,10))
    for(i_fp2m in 1:10)
    {
    	Size_FuncMeta_Aver[,i_fp2m] <- rowSums(Size_FuncMeta[,,i_fp2m])/N_Meta/k_num
    }
    write.table(Size_FuncMeta_Aver, "Meta_PPIDegPreserve_ThreeLayerModels_fpm01To1.txt",row.names = FALSE, col.names = FALSE)
