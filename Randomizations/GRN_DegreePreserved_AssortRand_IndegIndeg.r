...
for(i_rep_rewire in 1:N_PPI_link)
{
	...
	if(length(unique(c(A,B,C,D)))==4 && ((PPI_Degree[A]>PPI_Degree[C])&&(PPI_Degree[D]>PPI_Degree[B]) || (PPI_Degree[A]<PPI_Degree[C])&&(PPI_Degree[B]>PPI_Degree[D]))) 
	{	
    ...
    write.graph(g_PPI,"GRN_DegreePreserved_AssortRand_IndegIndeg.txt","edgelist")
   write.table(i_switch,"GRN_Switch_Number_AssortRand_IndegIndeg.txt",row.names=FALSE, col.names=FALSE)
}
