...
g_ori <- read.graph("GRN_DegreePreserved_AssortRand_IndegIndeg.txt", format = c("edgelist"),directed=TRUE)
g_Gene_read <- g_ori
...
#############################################     Metabolic network
    ...
    write.table(Size_FuncGene_Aver, "Gene_GRNIndegIndegAssort_ThreeLayerModels.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncPPI_Aver <- rowSums(Size_FuncPPI)/N_PPI/k_num
    write.table(Size_FuncPPI_Aver, "PPI_GRNIndegIndegAssort_ThreeLayerModels.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncMeta_Aver <-  array(rep(0,pmax*10), dim=c(pmax,10))
    for(i_fp2m in 1:10)
    {
    	Size_FuncMeta_Aver[,i_fp2m] <- rowSums(Size_FuncMeta[,,i_fp2m])/N_Meta/k_num
    }
    write.table(Size_FuncMeta_Aver, "Meta_GRNIndegIndegAssort_ThreeLayerModels_fpm01To1.txt",row.names = FALSE, col.names = FALSE)
