...
g_Gene_Ori <- g_ori
N_Gene_link <- length(E(g_Gene_Ori))
#g_PPI_Ori <- read.graph("PPI_Randomized_Assortative.txt", format = c("edgelist"),directed=FALSE)
...
{
	g_Gene <-  g_Gene_Ori
    for(i_rep_rewire in 1:N_Gene_link)
    {
        Edge2_ids <- sample(N_Gene_link,2,replace=FALSE)
        Endpoints22 <-ends(g_Gene, Edge2_ids, names = TRUE)
        ...
        if(Endpoints22[1,1]!=Endpoints22[2,1] && Endpoints22[1,1]!=Endpoints22[2,2] && Endpoints22[1,2]!=Endpoints22[2,1] && Endpoints22[1,2]!=Endpoints22[2,2]) 
        {
            ...
            End1ConnectEnd4 <- which(EndPints4[4] %in% as_ids(neighbors(g_Gene,EndPints4[1], mode=c("out"))))
            End2ConnectEnd3 <- which(EndPints4[2] %in% as_ids(neighbors(g_Gene,EndPints4[3], mode=c("out"))))
            if(length(End1ConnectEnd4)==0 && length(End2ConnectEnd3)==0)
            {
                g_Gene <- delete.edges(g_Gene, Edge2_ids )
                g_Gene <- add_edges(g_Gene, c(EndPints4[1], EndPints4[4]))
                g_Gene <- add_edges(g_Gene, c(EndPints4[3], EndPints4[2]))
            }
        }
     }		
...
#############################################     Metabolic network
    ...
    write.table(Size_FuncGene_Aver, "Gene_GeneRand.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncPPI_Aver <- rowSums(Size_FuncPPI)/N_PPI/k_num
    write.table(Size_FuncPPI_Aver, "PPI_GeneRand.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncMeta_Aver <-  array(rep(0,pmax*10), dim=c(pmax,10))
    for(i_fp2m in 1:10)
    {
    	Size_FuncMeta_Aver[,i_fp2m] <- rowSums(Size_FuncMeta[,,i_fp2m])/N_Meta/k_num
    }
    write.table(Size_FuncMeta_Aver, "Meta_GeneRand_fpm01To1.txt",row.names = FALSE, col.names = FALSE)
