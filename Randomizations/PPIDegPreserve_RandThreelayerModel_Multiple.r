...
for(k_num in 1:k_max)
{
    ...
    g_PPI <- g_PPI_Read
    ########## Randomize PPI
    for(i_rep_rewire in 1:N_PPI_link/2)
	{
		...
		{	
			AConnectC <- which(A %in% as_ids(neighbors(g_PPI,C, mode=c("all"))))
			BConnectD <- which(B %in% as_ids(neighbors(g_PPI,D, mode=c("all"))))
	    	if(length(AConnectC)==0 && length(BConnectD)==0)
	    	{
	    		g_PPI <- delete.edges(g_PPI, Edge2_ids )
				g_PPI <- add_edges(g_PPI, c(A, C))
				g_PPI <- add_edges(g_PPI, c(B, D))
			} else {
				AConnectD <- which(A %in% as_ids(neighbors(g_PPI,D, mode=c("all"))))
		    	BConnectC <- which(B %in% as_ids(neighbors(g_PPI,C, mode=c("all"))))
		    	if(length(AConnectD)==0 && length(BConnectC)==0)
	    		{
	    			g_PPI <- delete.edges(g_PPI, Edge2_ids )
					g_PPI <- add_edges(g_PPI, c(A, D))
					g_PPI <- add_edges(g_PPI, c(B, C))
				} 
			}
		}
	} 
...
#############################################     Metabolic network
    ...
    write.table(Size_FuncGene_Aver, "Gene_PPIDegPreserve_multple.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncPPI_Aver <- rowSums(Size_FuncPPI)/N_PPI/k_num
    write.table(Size_FuncPPI_Aver, "PPI_PPIDegPreserve_multple.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncMeta_Aver <-  array(rep(0,pmax*10), dim=c(pmax,10))
    for(i_fp2m in 1:10)
    {
    	Size_FuncMeta_Aver[,i_fp2m] <- rowSums(Size_FuncMeta[,,i_fp2m])/N_Meta/k_num
    }
    write.table(Size_FuncMeta_Aver, "Meta_PPIDegPreserve_multple_fpm01To1.txt",row.names = FALSE, col.names = FALSE)
    write.table(k_num, "RepeatNum_PPIDegPreserve_multple.txt",row.names = FALSE, col.names = FALSE)
