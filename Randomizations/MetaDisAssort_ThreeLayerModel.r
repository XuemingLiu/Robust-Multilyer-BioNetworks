...
PPI_Degree <- degree(g_PPI, V(g_PPI), mode = c("all"))       
Isolates <- union(which(Gene_Deg==0),which(PPI_Degree==0))
PPI_Isolates <- which(PPI_Degree==0)
...
Meta_Degree <- degree(Meta_Net, mode="all")
Meta_Isolates <- which(Meta_Degree==0)
N_Meta_link <- length(E(Meta_Net))
Meta_Net_Ori <- Meta_Net
...
GPInterNodes <- intersect(which(Gene_Deg!=0),which(PPI_Degree!=0))
...
{	
	Meta_Net <- Meta_Net_Ori
	for(i_rep_rewire in 1:N_Meta_link)
	{
		Edge2_ids <- sample(N_Meta_link,2,replace=FALSE)
		Endpoints22 <-ends(Meta_Net, Edge2_ids, names = TRUE)
		EndpointsArrary <- c(Endpoints22[1,1],Endpoints22[2,1],Endpoints22[1,2],Endpoints22[2,2])	
		EndpointsArrary 	
		if(Endpoints22[1,1]!=Endpoints22[2,1] && Endpoints22[1,1]!=Endpoints22[2,2] && Endpoints22[1,2]!=Endpoints22[2,1] && Endpoints22[1,2]!=Endpoints22[2,2]) 
		{
			EndPints4 <- EndpointsArrary[order(-Meta_Degree[EndpointsArrary])]		
			End1ConnectEnd4 <- which(EndPints4[4] %in% as_ids(neighbors(Meta_Net,EndPints4[1], mode=c("all"))))
			End2ConnectEnd3 <- which(EndPints4[2] %in% as_ids(neighbors(Meta_Net,EndPints4[3], mode=c("all"))))
	    	if(length(End1ConnectEnd4)==0 && length(End2ConnectEnd3)==0)
	   		 {
	    		Meta_Net <- delete.edges(Meta_Net, Edge2_ids )
				Meta_Net <- add_edges(Meta_Net, c(EndPints4[1], EndPints4[4]))
				Meta_Net <- add_edges(Meta_Net, c(EndPints4[2], EndPints4[3]))
			}
		}
	}
...
#############################################     Metabolic network
    ...
    write.table(Size_FuncGene_Aver, "Gene_MetaDisAssort.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncPPI_Aver <- rowSums(Size_FuncPPI)/N_PPI/k_num
    write.table(Size_FuncPPI_Aver, "PPI_MetaDisAssort.txt",row.names = FALSE, col.names = FALSE)
    Size_FuncMeta_Aver <-  array(rep(0,pmax*10), dim=c(pmax,10))
    for(i_fp2m in 1:10)
    {
    	Size_FuncMeta_Aver[,i_fp2m] <- rowSums(Size_FuncMeta[,,i_fp2m])/N_Meta/k_num
    }
    write.table(Size_FuncMeta_Aver, "Meta_MetaDisAssort_fpm01To1.txt",row.names = FALSE, col.names = FALSE)
