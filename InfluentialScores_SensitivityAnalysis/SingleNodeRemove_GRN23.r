# calculate the influence scores in themultilayer netowrks including tissue specific GRN23
# Output: final functional sizes in the PPI network

library(igraph)

g_ori <- read.graph("23_lung_Num.txt", format = c("edgelist"), directed=TRUE)
G_GRN_Read <- g_ori
...
PPI_Nodes_Robust[,2] <- sort(PPIFunc_NonIso) 
    ...
    write.table(PPI_Nodes_Robust, "SingleAttack_PPI_Robust_GRN23.txt",row.names = FALSE, col.names = FALSE)      
